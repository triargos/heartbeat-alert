...
export class Heartbeat {
  ...
  private extractMonitors(
    data: Awaited<ReturnType<typeof this.fetchMonitors>>,
  ) {
    const monitors: Monitor[] = [];
    if (
      data.aggregations &&
      data.aggregations.by_monitors &&
      data.aggregations.by_monitors.buckets
    ) {
      ...
      for (const bucket of buckets) {
        if (
          bucket.top_hit &&
          bucket.top_hit.hits &&
          bucket.top_hit.hits.hits.length > 0
        ) {
