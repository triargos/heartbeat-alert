import {
  ...
  getLastMessageStatusForChannel,
  getLastMonitorActions,
  logMessage,
} from "@packages/db";
...
export async function notifyErrorMessage(
  channel: CHANNEL,
  status: ACTION,
  message?: string,
) {
    ...
    for (const channel of rule.channels) {
      ...
      if (!lastMessage || lastMessage?.status !== status) {
        ...
        const diff = DateTime.now()
          .diff(DateTime.fromISO(lastMessage.timestamp))
          .as("hours");
...
export async function shouldNotifyErrorMessage(status: ACTION) {
  const rules = readRules().filter((rule) => rule.event === status);
  const channelsToNotify = await Promise.all(
    rules.map(async (rule) => {
      return await Promise.all(
        rule.channels.map(async (channel) => {
          //Check if we already sent an error
          const lastErrorMessage = await getLastMessageStatusForChannel(
            channel,
            status,
          );
          if (!lastErrorMessage) {
          ...
          const diff = DateTime.now()
            .diff(DateTime.fromISO(lastErrorMessage.timestamp))
            .as("hours");
          if (diff > 12 && status === ACTIONS.STATUS_DOWN) {
            return channel;
          }
